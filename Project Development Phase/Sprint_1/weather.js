...
infoTxt = inputPart.querySelector(".info-txt"); // please enter... kismi
inputField = inputPart.querySelector("input"); // input alani
...
locationBtn.addEventListener("click", () => {
    //eger tarayici lokasyonu destekliyorsa
...
function requestApi(city){
    //api alip json object dönüstürme
...
function onError(error){
    infoTxt.innerText = error.message; //tarayicinin verecegi error ekrana basar.
...
function fetchData(){
    ...
    //then func. api sonucu ile weatherDetails çagirir.
};
...
function weatherDetails(info){
    ...
    const {feels_like, humidity, temp} = info.main;
        ...
        if(id == 800){
            ...
            wIcon.src="cloudy.svg"
        }  else if ((id >= 300 && id <= 321) || (id >= 500 && id <= 531)){
            wIcon.src="rainy.svg"
        } 

   //html'e basmamiz için
